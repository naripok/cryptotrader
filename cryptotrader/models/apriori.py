...
def price_relative(obs, period=1):
    ...
    price_relative = prices.apply(ta.ROCR, timeperiod=period, raw=True).fillna(1.0) - 1
...
def tsf(obs, period=14):
    prices = obs.xs('open', level=1, axis=1).astype(np.float64).apply(ta.ROCR, timeperiod=1, raw=True).fillna(0) - 1
    tsf = prices.apply(ta.TSF, timeperiod=period, raw=True).fillna(0.0)
    return tsf